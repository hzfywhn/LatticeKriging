...
[Q, phi] = combineMR(struct('loc', [x y], 'azim', [cos(t) sin(t)]), basis, normalization, rho, true);

nl = 20;
lamb = logspace(2, 4, nl);
like = zeros(nl, 1);
for i = 1: nl
    [~, ~, ~, l, ~] = kriging(lamb(i), yfit, W, Z, Q, phi);
    like(i) = l;
end
figure
semilogx(lamb, like)

[l, ~, ~, ~] = findblup(yfit, W, Z, Q, phi, 4, 9, optimset('FunValCheck', 'on'));
lambda = exp(l);
